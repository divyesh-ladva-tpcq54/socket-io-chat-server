...
export class GroupMessageHandler {
  ...
  public handler = async (io: Server, socket: Socket, data: IGroupMessageData) => {
    try {
      ...
      if (error || !receiverUser) {
        ...
        socket.emit(SocketEvents.groupMessage, responseData);
        return;
      }
  ...
  private async socketOperations(io: Server, socket: Socket, receiverUser: User, data: IGroupMessageData) {
  ...
  private async dbOperations(socket: Socket, data: IGroupMessageData) {
    // check if receiver exists on the system 
    // @ts-ignore
    const receiverUser = await this.userService.findOne(data);
